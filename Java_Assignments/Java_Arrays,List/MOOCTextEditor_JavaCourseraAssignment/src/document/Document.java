...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		{
		    if(i==cArray.length-1 && Character.toLowerCase(cArray[i])=='e' && newSyllable && count > 0) {
                count--;
            }
	...
	public double getFleschScore()
	{
		...
		int syllablesCount=getNumSyllables();
		if(wordsCount==0 || sentencesCount==0 || syllablesCount==0)
			return 0.0;
		double fleschScore=206.835-1.015*((double)wordsCount/(double)sentencesCount)-84.6*((double)syllablesCount/(double)wordsCount);
		return fleschScore;
	}
