...
You should group all words of length 30 or more together, and you should not count basic punctuation that are the
 first or last characters of a group of characters.
...
an integer array named counts. This method should read in the words from resource and count the number of words
...
- If a word has a non-letter as the first or last character, it should not be counted as part of the word length.
 ...
 Note that we will miscount some words, such as “Hello,” which will be counted as 6 since we don’t count the
 double quotes but will count the comma, but that is OK as there should not be many words in that category.
...
and creates a counts integer array of size 31. This method should call countWordLengths with a file
and then print the number of words of each length. Test it on the small file smallHamlet.txt shown below.
...
the most common word length in the file.
...
public class WordLengths {
    public static void countWordLenghts(FileResource fr,int[] counts){
        for(String words: fr.words()){
            if(!Character.isLetter(words.charAt(words.length()-1)) || !Character.isLetter((words.charAt(0))))
                counts[words.length()-1]+=1;
            else
    ...
    public static void testCountWordLengths(){
        ...
        countWordLenghts(fr,counts);
        int k=indexOfmax(counts);
        System.out.println("Maximum Index Value: "+k);
    }
