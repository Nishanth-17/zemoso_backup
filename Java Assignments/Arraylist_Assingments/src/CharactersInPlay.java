...
Here are a few lines from the file put into a much smaller file called macbethSmall.txt:
...
Note that each speaking part is at the beginning of the line (there may be some blanks before it) and
has a period immediately following it. Shakespeare used this format in many of his plays.
...
those with the most speaking parts. You should identify a speaking part by reading the file line-by-line and
finding the location of the first period on the line. Then you will assume that everything up to the first period
is the name of a character and count how many times that occurs in the file.
...
Periods that indicate the end of a sentence will likely be a unique phrase so you won’t print that as
it would just occur once or maybe twice.

For the file macbethSmall.txt, if we process it and print ALL the possible speaker characters and counts found,
...
Put all the following items below in this class.
You will need to create two private ArrayLists.
One to store the the names of the characters you find and one to store the corresponding counts for each character.
...
This method should call findAllCharacters, and then for each main character, print out the main character,
...
where you can assume num1 should be less than or equal to num2. This method should print out the names of
all those characters that have exactly number speaking parts, where number is greater than or equal to num1 and
less than or equal to num2. Add code in tester to test this method out.
 */
...
public class CharactersInPlay {
    ...
    public void update(String person){
        if(!CharacterNames.contains(person)) {
            ...
            CharacterCount.set(CharacterNames.indexOf(person),CharacterCount.get(CharacterNames.indexOf(person))+1);
    }
    ...
    public void tester() {
        ...
        for (int i = 0; i < CharacterNames.size(); i++) {
            if(CharacterCount.get(i)>=CharacterCount.get(Max))
                Max=i;
        }
        System.out.println(CharacterNames.size());
        System.out.println(CharacterNames.get(Max) + " = " + CharacterCount.get(Max));
